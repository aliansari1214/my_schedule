...
class Job:
    ...
    def hour(self):
        if self.interval != 1:
            return IntervalError('use hours instead of hour')
        return self.hours
    ...
    def at(self,time_str):
        ...
        minute = int(minute)
        if not 0<= minute <= 59:
            raise ScheduleValueError(f'Invalid format time "{time_str}"')
        if self.unit == 'days':
            hour = int(hour)
            if not 0 <= hour <= 23:
                return ScheduleValueError(f'Invalid format time "{time_str}"')
    ...
    def _schedule_next_run(self):
        if self.unit not in ('seconds','minutes','days','hours'):
            raise ScheduleValueError('invalid unit')
...
def every(interval=1):
    return default_schedule.every(interval)


def run_pending():
    return default_schedule.run_pending()


def run_all(delay_seconds=0):
	default_schedule.run_all(delay_seconds)

def next_run():
	return default_schedule.next_run

def idle_seconds():
	return default_schedule.idle_seconds
